  ...
  policy = {
    ...
    services = {
      sos = {
        ...
        rules = [
          {
            # Allow listing objects under hr/ in any bucket
            expression = "operation == 'list-objects'"
            action     = "allow"
          },
          {
            # Allow read/write only on keys that start with hr/
            expression = "operation in ['get-object', 'put-object', 'delete-object'] && parameters.key.startsWith('hr/')"
            action     = "allow"
          }
        ]
      }
    ...
    services = {
      sos = {
        ...
        rules = [
          ...
          {
            # Allow read/write only on keys that start with hr/
            expression = "operation in ['get-object', 'put-object', 'delete-object'] && parameters.key.startsWith('sales/')"
            action     = "allow"
          }
        ]
      }
